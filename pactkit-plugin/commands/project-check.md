---
description: "Execute project-check through Core-owned bounded WorkUnits"
allowed-tools: [Read, Write, Edit, Bash, Glob, Grep]
---

# Command: project-check — Managed WorkUnit Facade
- **Usage**: /project-check "$ARGUMENTS"


## Pre-Final Protocol (MUST)
Run and obey `pactkit workflow contract project-check --json`. Before final run `pactkit workflow finish-guard <run-id> --json`: `continue_current_turn` means continue; only `done`/`await_user` ends. Progress is not final.

Core is the only workflow scheduler and completion authority for this command.

1. Start with `pactkit work-unit start project-check --goal "$ARGUMENTS" --story-id <story-id>`.
