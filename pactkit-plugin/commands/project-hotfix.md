---
description: "Execute project-hotfix through Core-owned bounded WorkUnits"
allowed-tools: [Read, Write, Edit, Bash, Glob, Grep]
---

# Command: project-hotfix — Managed WorkUnit Facade
- **Usage**: /project-hotfix "$ARGUMENTS"


## Pre-Final Protocol (MUST)
Run and obey `pactkit workflow contract project-hotfix --json`. Before final run `pactkit workflow finish-guard <run-id> --json`: `continue_current_turn` means continue; only `done`/`await_user` ends. Progress is not final.

Core is the only workflow scheduler and completion authority for this command.

1. Start with `pactkit work-unit start project-hotfix --goal "$ARGUMENTS" --story-id <story-id>`.
